...
TimescaleDB. **Grafana presents the data** - this repo deliberately does not. The project was pruned
back to that scope; do not reintroduce dashboards, charts, history views or an SPA frontend.

```
sensors -> weather_daq -> spool (sqlite) -> TimescaleDB -> Grafana
...
- **`weather_web/`** - one Flask page (`app.py`, ~90 lines) showing current readings and sensor
  health, plus `/api/current` and `/healthz`. It reads the DAQ's status JSON file - **no database
  access at all** - so it is fast and survives a TimescaleDB outage. One template, no static assets,
  no JS. `WEATHER_STATUS_FILE` must match `STATUS_FILE` in the DAQ settings.
...
- `weather_daq/settings.py` is **gitignored**; `example_settings.py` is the template. Copy and fill in.
- `DB_SETTINGS` holds `DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD` (note: `DB_PASSWORD`, not the
  old `DB_PASSWD`).
- The DAQ imports the Adafruit Blinka/`board` stack, which only works on real Pi hardware. Edit here,
  ...
  run and tested on the Windows dev machine.
